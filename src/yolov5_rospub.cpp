...
std::string my_classes[]={ "fire", "smoke", "person" };

//conpress the input "image" ,and return a sensor_msgs::CompressedImage
// sensor_msgs::CompressedImage cv_to_ros(cv::Mat& image ,int width,int height) // 将cv图像格式转换为ROS压缩图像格式
// {
//     cv::Mat cvCompImg;
//     cv::resize(image,cvCompImg,cv::Size(width,height));
//     sensor_msgs::ImagePtr compImg = cv_bridge::CvImage(std_msgs::Header(),"rgb8",cvCompImg).toImageMsg();
//     return compImg->image;
// }
...
int main(int argc, char** argv) {
    ...
    if(!parse_args(argc,argv,engine_name)){
        ...
        std::cerr << "./yolov5_ros -v [.engine] // run inference with camera" << std::endl;
        return -1;
    }
    ...
    //帧数
    int fcount = 0;
    std::cout << "rdws is detecting......" << std::endl;
    while(ros::ok())
    {
        cv::Mat frame;
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
				...
				target.y = r.y + r.height/2;
                target_array.targets.push_back(target);
                // ROS_INFO("detect---------[%d]", j);
                // ROS_INFO("target_id:   [%d]", target_array.targets[j].id);
                // ROS_INFO("target_prob:   [%f]", target_array.targets[j].prob);
                // ROS_INFO("target_x:   [%d]",target_array.targets[j].x);
                // ROS_INFO("target_y:   [%d]", target_array.targets[j].y);
            }
        ...
        //cv::resizeWindow("yolov5", 2700, 900);    // 设置长和宽
        //cv::imshow("yolov5",frame);
        
        //加工并发送图片消息
        sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", frame).toImageMsg();

        //std::cout << "本轮检测结果数：" << batch_res[0].size() << std::endl;
        //同步两个话题信息的时间戳
        target_array.header.stamp = msg->header.stamp = ros::Time::now();

        //publish targetarray：若此轮检测结果数大于零（本次采样中的第一帧，其实本来就只有一帧），则进行检测列表的消息加工和发送
        if(batch_res[0].size() > 0){
            //target_array.header.stamp = ros::Time::now();
            //target_array.image = cv_to_ros(frame,900,300);
            pubTarget.publish(target_array);
        }

        pubDetectImage.publish(msg);

        std::cout << "本轮检测的是时间："<<target_array.header.stamp.nsec<<"图片时间:"<<msg->header.stamp.nsec<<std::endl;
